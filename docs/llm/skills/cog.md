...
For each non-system domain you created or reconfigured (skip `cog-meta`), write a skill file with the host's **local (non-cog) `write` tool** into the host's skills directory. Overwrite on re-run — this template is the source of truth. Fill in from the domain's manifest entry:

> **HOST-ADAPTATION**: The host must provide a local file-write mechanism and a skills directory where generated domain skills are picked up for session routing — the write goes through the host's local tools, NOT cog memory. (reference host: ytsejam writes `skills/{id}.md` with its local `write` tool; the path is relative to the assistant's data directory, not the cog memory root.)
